...
# -- flags
var is_left_mouse_held_down := false
var is_right_mouse_held_down := false
...
func _process(delta):
	...
	if within_bounds(tile_pos):
		...
		clear_hover_tiles()
		map.set_cell(2, tile_pos, 0, Vector2i(7, 7))
	else:
		tile_pos_label.hide()
		clear_hover_tiles()
	...
	if is_right_mouse_held_down:
		clear_tile(get_tile_under_cursor())
...
func choose_room_type(type):
	...
	# -- hide all room type selector sprites
	for c in room_type_container.get_children():
		c.selector.hide()
	
	# -- show current room type selector sprite
	room_type_container.get_child(room_type).selector.show()
...
func set_up_tile_buttons():
	...
	for t in tile_count:
		...
		if tile_coord.y != 7 and tile_coord != Vector2i(0, 0):
			# add to tile_to_atlas dict
			tile_to_atlas[t - skipped_tiles] = tile_coord
...
func clear_tile(tile_pos) -> void:
	if within_bounds(tile_pos):
		map.set_cell(1, tile_pos)

func change_tile(tile_pos) -> void:
	if selected_tile > -1 and within_bounds(tile_pos):
		map.set_cell(1, tile_pos, 0, tile_to_atlas[selected_tile])
...
func update_tiles() -> void:
	for x in room_size.x:
		for y in room_size.y:
			map.set_cell(1, Vector2i(x, y), 0, tile_to_atlas[tiles_dict[Vector2i(x, y)]])
...
func select_tile(tile):
	...
	# -- show current tile selector
	button_container.get_child(selected_tile).selector.show()

func clear_hover_tiles() -> void:
	for x in room_size.x:
		for y in room_size.y:
			map.set_cell(2, Vector2i(x, y))
